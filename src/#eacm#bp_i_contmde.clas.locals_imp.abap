CLASS lhc_I_CONTMDE DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

*    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
*      IMPORTING keys REQUEST requested_authorizations FOR /eacm/i_contmde RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR /eacm/i_contmde RESULT result.

*    METHODS create FOR MODIFY
*      IMPORTING entities FOR CREATE /eacm/i_contmde.
*
*    METHODS update FOR MODIFY
*      IMPORTING entities FOR UPDATE /eacm/i_contmde.
*
*    METHODS delete FOR MODIFY
*      IMPORTING keys FOR DELETE /eacm/i_contmde.

    METHODS read FOR READ
      IMPORTING keys FOR READ /eacm/i_contmde RESULT result.

    METHODS lock FOR LOCK
      IMPORTING keys FOR LOCK /eacm/i_contmde.
    METHODS postmde FOR MODIFY
      IMPORTING keys FOR ACTION /eacm/i_contmde~postmde RESULT result.

ENDCLASS.

CLASS lhc_I_CONTMDE IMPLEMENTATION.
...
METHOD PostMde.
  DATA(lo_posting) = NEW /eacm/cl_mde_posting( ).

  LOOP AT keys ASSIGNING FIELD-SYMBOL(<key>).
    DATA(ls_selection) = VALUE /eacm/cl_mde_posting=>ty_selection(
    ...
    IF <key>-%param-DocumentDateFrom IS NOT INITIAL
       OR <key>-%param-DocumentDateTo IS NOT INITIAL.
      APPEND VALUE #(
        sign   = 'I'
        option = COND #( WHEN <key>-%param-DocumentDateTo IS INITIAL THEN 'EQ' ELSE 'BT' )
        low    = <key>-%param-DocumentDateFrom
        high   = <key>-%param-DocumentDateTo ) TO ls_selection-document_date_range.
    ENDIF.

    DATA lt_posting_result TYPE /eacm/cl_mde_posting=>tt_result.
    ...
    LOOP AT lt_posting_result INTO DATA(ls_posting_message).
      ...
      APPEND VALUE #(
        ...
        %msg = new_message_with_text(
          ...
          text = ls_posting_message-message_text ) ) TO reported-/eacm/i_contmde.
    ENDLOOP.

    DATA(lv_message_text) = COND string(
      WHEN lv_message_type = 'E'
      THEN |Contabilizzazione MDE terminata con errori: { lv_error_count }.|
      WHEN lv_message_type = 'W'
      THEN |Contabilizzazione MDE terminata con avvisi: { lv_warning_count }.|
      WHEN lv_success_count > 0
      THEN |Contabilizzazione MDE completata: { lv_success_count } righe sorgente aggiornate.|
      ELSE 'Contabilizzazione MDE completata.' ).

    IF lv_message_type = 'E'.
      APPEND VALUE #( %cid = <key>-%cid ) TO failed-/eacm/i_contmde.
    ENDIF.
...
CLASS lsc_I_CONTMDE DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_I_CONTMDE IMPLEMENTATION.
